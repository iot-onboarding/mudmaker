...
1. Open `mudmaker.html`.
   The page opens on the Create tab and initializes a working MUD object in browser session storage. The initial object includes defaults such as MUD version, cache validity, support status, and OpenChain metadata.
...
8. Save or continue work.
   In the Publish/Save/Continue Work tab, the user can save the current work as a local JSON file. Later, the user can upload that saved JSON file through Continue Earlier Work to reload the form and continue editing.
